...
19. 做完任务的时候请一定要检查一下当前页面是否有未完成的任务要求，如果有请继续完成，如果没有了再执行 finish，在执行finish的时候要确认是否真正已完成，如未完成需要重试，比如发送消息，需要查看发送消息的时候是否和预期一致，如未完成请返回之前的步骤进行纠正。
20. 每次做完一个操作一定要总结一下当前页面的状态，确认是否有新的信息可以利用，或者是否有需要纠正的操作，如果有请继续操作，如果没有了再执行下一步操作。
"""
)
